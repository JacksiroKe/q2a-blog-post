...
if (!defined('QA_VERSION')) { // don't allow this page to be requested directly from browser
	...
	class blog_post
	{
		...
		public function process_request( $request )
		{
			...
			$catslugs = qa_request_parts(2);
			$blogurl = qa_request_part(1);
			$countslugs = count($catslugs);
			$userid = qa_get_logged_in_userid();
			$blog_cats = qa_db_select_with_pending(bp_db_cat_nav_selectspec(null, true, false, true));
			if (is_numeric( $blogurl )) {
				...
				switch ( $blogurl ) {
					case 'write':
						...
						break;
					default:
						$qa_content = $this->qa_blog_home( $userid, $catslugs, $countslugs );
						break;
				}
			}
						...
						foreach ($recentArticles as $article) {
							if (!$article['hidden']) {
								...
								if ($pTitleWords == $testTitleWords && $pContentWords == $testContentWords) {
									$errors['page'] = qa_lang_html('article/duplicate_content');
									break;
								}
							}
						}
		...
		public function qa_blog_post( $userid, $permalink )
		{
			...
			if (!count($findpost)) return $this->bp_article_not_found();
			...
			if ($article['basetype'] != 'P') $article = null;
			if (isset($article)) {
				$p_request = bp_p_request($permalink);
				if (trim($p_request, '/') !== trim(qa_request(), '/')) qa_redirect($p_request);
			...
			if (!isset($article)) return $this->bp_article_not_found();
			...
			if (!$formrequested && $article['answerbutton']) {
				...
				if ($immedoption == 'always' || ($immedoption == 'if_no_as' && !$article['isbyuser'] && !$article['acount']))
			...
			if ($formtype == 'a_edit') {
				...
				$qa_content['a_form'] = qa_page_q_add_a_form($qa_content, 'anew', $captchareason, $article, @$anewin, @$anewerrors, $formtype == 'a_add', $formrequested);
