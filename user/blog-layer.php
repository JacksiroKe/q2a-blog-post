...
class qa_html_theme_layer extends qa_html_theme_base
{
	function nav_list($navigation, $class, $level = null)
	{
		...
		$isPermitted = qa_user_maximum_permit_error('bp_permit_post_p', QA_LIMIT_QUESTIONS);

		$userTemplates = array('user', 'user-wall', 'user-activity', 'user-questions', 'user-answers', 'user-articles', 'account', 'messages', 'favorites');

		if (in_array($this->template, $userTemplates)) {
			if ($class == 'nav-sub' && (qa_admin_check_privileges($qa_content) || qa_get_logged_in_handle() == $handle || !$isPermitted)) {
				$navigation['articles'] = array(
					'label' => qa_lang('bp_lang/nav_articles'),
					'url' => qa_path_html('user/' . $handle . '/articles'),
					'selected' => (strtolower(qa_request_part(2)) == 'articles') ? 'selected' : '',
				);
			}
		}
	...
	function bp_user_articles($handle, $userhtml)
	{
		...
		if (count($articles))
			$qa_content['title'] = qa_lang_html_sub('bp_lang/articles_by_x', $userhtml);
		else
			$qa_content['title'] = qa_lang_html_sub('bp_lang/no_articles_by_x', $userhtml);
		...
		foreach ($articles as $article) {
			$qa_content['q_list']['qs'][] = bp_post_html_fields(
				$article,
				$loginuserid,
				qa_cookie_get(),
				$usershtml,
				null,
				qa_post_html_options($article, $htmldefaults)
			);
		}
